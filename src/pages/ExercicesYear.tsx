...
const ExercicesYear = () => {
  ...
  if (!stage || !year) return <Navigate to="/exercices" replace />;
  ...
  return (
    <SiteLayout>
      <PageHero
        ...
        title={`تمارين ${year.title}`}
        description="تمارين مرتبة حسب الوحدة. اضغط على وحدة للاطلاع على تمارينها."
        crumbs={[
          { label: "التمارين", to: "/exercices" },
          { label: stageLabel, to: `/exercices/${stage.id}` },
          { label: year.title },
        ]}
      />
